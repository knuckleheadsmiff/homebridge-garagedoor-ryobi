import { AccessoryConfig, AccessoryPlugin, API, Logging } from "homebridge";
...
class GarageCmdAccessory implements AccessoryPlugin {
  constructor(logger: Logging, config: AccessoryConfig, api: API) {
    ...
    this.garagedoor = new ryobi_GDO_API(
      this.ryobi_email,
      this.ryobi_password,
  ...
  setState(targetState, callback, context) {
    ...
    var accessory = this;
    var command =
      targetState == Characteristic.CurrentDoorState.CLOSED
        ? accessory.garagedoor.closeDoor.bind(accessory.garagedoor)
        : accessory.garagedoor.openDoor.bind(accessory.garagedoor);
      ...
      if (err) {
        ...
        this.debug(
          "State of Characteristic.CurrentDoorState[state] is: " +
            Characteristic.CurrentDoorState[state]
        );
  ...
  pollState() {
    ...
    var doIt = function (checkQuick) {
      accessory.getState(
        function (err, currentDeviceState) {
          try {
            if (err) {
              this.debug(
                "pollState (" + accessory.poll_short_delay + "):" + err
              );
              accessory.stateTimer = setTimeout(
                doIt,
                accessory.poll_long_delay
              );
              return;
            }
            this.debug(
            ...
            if (
              currentDeviceState == Characteristic.CurrentDoorState.OPENING ||
              currentDeviceState == Characteristic.CurrentDoorState.CLOSING
            ) {
            ...
            accessory.log(error);
            accessory.stateTimer = setTimeout(doIt, accessory.poll_state_delay);
          }
        }.bind(this)
      );
    }.bind(this);
